...
export async function getFirmQuote(this: IExecuteFunctions) {
    ...
    if ((sellDeliveryMethod && buyDeliveryMethod) || (!sellDeliveryMethod && !buyDeliveryMethod)) {
        throw new Error('Either "Sell delivery method" or "Buy delivery method" should have a value, not both.');
    }
    ...
    if(showOptionalValues){
        ...
        firmQuoteRequest = new QuotePostRequest({ sellAsset, sellAmount, buyAsset, buyAmount, context, expireAfter, sellDeliveryMethod, buyDeliveryMethod, countryCode });
    }else{

        firmQuoteRequest = new QuotePostRequest({ sellAsset, sellAmount, buyAsset, buyAmount, context, sellDeliveryMethod, buyDeliveryMethod });
    }
